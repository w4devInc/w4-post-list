		...
		case "html_input":
			...
			if ($type == 'text') {
				$html .= sprintf(
					...
					w4pl_form_pitc_class('wff', $id, $type), $id, $name, esc_attr($value), $input_class, $placeholder
				);

			} elseif ($type == 'textarea') {
				$html .= sprintf(
					'<textarea class="%1$s %5$s" id="%2$s" name="%3$s" placeholder="%6$s">%4$s</textarea>',
					w4pl_form_pitc_class('wff', $id, $type), $id, $name, esc_textarea($value), $input_class, $placeholder
				);

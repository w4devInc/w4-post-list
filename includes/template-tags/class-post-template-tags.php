...
class W4PL_Post_Template_Tags
{
	...
	public static function template_title($attr, $cont)
	{
		return sprintf(
			...
			get_permalink(),
			sprintf(esc_attr__('View %s', 'w4-post-list'), the_title_attribute('echo=0')),
			get_the_title()
		);
	}
	...
	public static function template_author($attr, $cont)
	{
		return sprintf(
			...
			sprintf(esc_attr__('View all posts by %s', 'w4-post-list'), get_the_author()),
			get_the_author()
		);
	}
	public static function template_excerpt($attr, $cont)
	{
		return sprintf(
			'<div class="post-excerpt">%s</div>',
			self::post_excerpt($attr, $cont)
		);
	}
